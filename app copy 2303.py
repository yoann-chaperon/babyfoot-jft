...
@app.route("/")
def index():
    # Joueurs actifs interclub
    interclub_actifs = Player.query.filter_by(
        membre_club=True,
        interclub=True,
        actif=True
    ).all()

    # Classement Duel et Goal
    classement_duel = sorted(interclub_actifs, key=lambda j: j.duel, reverse=True)
    classement_goal = sorted(interclub_actifs, key=lambda j: j.goal, reverse=True)

    # Legend
    legend = classement_duel[0] if classement_duel else None

    # Meilleur Buteur
    meilleur_buteur = classement_duel[1] if len(classement_duel) > 1 else classement_duel[0]

    # Meilleur Goal
    meilleur_goal = classement_goal[1] if len(classement_goal) > 1 else classement_goal[0]

    # -------------------------------
    # Assignation automatique des vidéos
    # -------------------------------
    for joueur in interclub_actifs:
        if legend and joueur.id == legend.id:
            joueur.card_video = "/static/videos/legend.mp4"
        elif meilleur_buteur and joueur.id == meilleur_buteur.id:
            joueur.card_video = "/static/videos/buteur.mp4"
        elif meilleur_goal and joueur.id == meilleur_goal.id:
            joueur.card_video = "/static/videos/goal.mp4"
        else:
            joueur.card_video = None
    ...
    return render_template(
        ...
        last_duel=last_duel,
        last_goal=last_goal,
        interclub_actifs=interclub_actifs,  # <-- tu passes les joueurs avec card_video
        legend=legend,
